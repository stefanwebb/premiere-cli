// Command: set-clip-rotation → ppb_setClipRotation
...
// premiere-pro-mcp `set_clip_rotation` tool (track-targeting.ts), addressed
...
// Rotation property (degrees; values beyond 0-360 are allowed for multiple
// rotations). Mutating — undo is non-functional on this build, so
// previousValue in the result is the caller's only restoration path.

function ppb_setClipRotation(argsJson) {
  try {
    var args;
    try {
    ...
    if (typeof args.degrees !== "number") {
      return JSON.stringify({ ok: false, error: "degrees must be a number" });
    }
    ...
    var setResult = setComponentProperty(motion, ["Rotation"], args.degrees);
    if (!setResult.found) {
      return JSON.stringify({ ok: false, error: "Rotation property not found on the Motion component" });
    }
