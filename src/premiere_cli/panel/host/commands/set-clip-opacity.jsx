// Command: set-clip-opacity → ppb_setClipOpacity
...
// premiere-pro-mcp `set_clip_opacity` tool (track-targeting.ts), addressed
// here by trackType/trackIndex/clipIndex (same convention as
// get-full-clip-info) instead of node_id. Sets the Opacity component's
// Opacity property (0-100). Mutating — undo is non-functional on this
// build, so previousValue in the result is the caller's only restoration
// path.

function ppb_setClipOpacity(argsJson) {
  try {
    var args;
    try {
    ...
    if (typeof args.opacity !== "number" || args.opacity < 0 || args.opacity > 100) {
      return JSON.stringify({ ok: false, error: "opacity must be a number between 0 and 100" });
    }
    ...
    var setResult = setComponentProperty(opacityComp, ["Opacity"], args.opacity);
    if (!setResult.found) {
      return JSON.stringify({ ok: false, error: "Opacity property not found on the Opacity component" });
    }
