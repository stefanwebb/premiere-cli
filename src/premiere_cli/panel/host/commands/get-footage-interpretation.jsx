// Command: get-footage-interpretation → ppb_getFootageInterpretation
...
function ppbFindItemGetFootageInterp_walk(item, args, depth) {
  if (depth > 32) {
    return null;
  }
  ...
  if (isBin && item.children) {
    for (var i = 0; i < item.children.numItems; i++) {
      var found = ppbFindItemGetFootageInterp_walk(item.children[i], args, depth + 1);
      if (found) {
        return found;
      }
    }
...
function ppbFindItemGetFootageInterp_resolve(args) {
  var root = app.project.rootItem;
  for (var i = 0; i < root.children.numItems; i++) {
    var found = ppbFindItemGetFootageInterp_walk(root.children[i], args, 1);
    if (found) {
      return found;
    }
  }
...
function ppb_getFootageInterpretation(argsJson) {
  try {
    var args;
    try {
    ...
    var item = ppbFindItemGetFootageInterp_resolve({
    ...
    try { result.alphaUsage = interp.alphaUsage; } catch (e) { result.alphaUsage = null; }
    try { result.fieldType = interp.fieldType; } catch (e) { result.fieldType = null; }
    try { result.frameRate = interp.frameRate; } catch (e) { result.frameRate = null; }
    try { result.ignoreAlpha = interp.ignoreAlpha; } catch (e) { result.ignoreAlpha = null; }
    try { result.invertAlpha = interp.invertAlpha; } catch (e) { result.invertAlpha = null; }
    try { result.pixelAspectRatio = interp.pixelAspectRatio; } catch (e) { result.pixelAspectRatio = null; }
