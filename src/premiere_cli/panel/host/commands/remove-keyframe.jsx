// Command: remove-keyframe → ppb_removeKeyframe
...
// `remove_keyframe` tool (keyframes.ts), addressed here by
// trackType/trackIndex/clipIndex + componentName/propertyName. `seconds`
// is clip-relative, offset by clip.start to sequence time. Tolerance is
// fixed at HALF A FRAME (0.5 / sequence fps) either side of the requested
// time, per the task spec — this is the cleanup path for add-keyframe
// mistakes (undo is non-functional on this build), so it first tries to
// locate an EXISTING key within tolerance via getKeys() (removing that
// exact key object, most likely to succeed), falling back to constructing
// a fresh time argument at the requested position via tryTimeForms() if no
// existing key matched or getKeys() itself is unavailable. Verified via a
// getKeys() COUNT drop, not by confirming which key vanished.
function ppb_removeKeyframe(argsJson) {
  try {
    var args;
    try {
    ...
    var rawKeys = null;
    try {
      rawKeys = prop.getKeys();
      keyCountBefore = rawKeys ? rawKeys.length : 0;
    } catch (e) {
      keyCountBefore = null;
    }
    ...
    if (matchedKey !== null) {
      try {
        prop.removeKey(matchedKey);
        removeAttempts.push({ form: "existingKeyObject", success: true });
        removed = true;
      } catch (e2) {
        removeAttempts.push({ form: "existingKeyObject", success: false, error: e2.toString() });
      }
    }
    ...
    if (!removed) {
      ...
      removeAttempts = removeAttempts.concat(tf.attempts);
      removed = tf.success;
    }
