// Command: set-clip-scale → ppb_setClipScale
...
// premiere-pro-mcp `set_clip_scale` tool (track-targeting.ts), addressed
...
// Scale property (100 = original size). Mutating — undo is non-functional
// on this build, so previousValue in the result is the caller's only
// restoration path.

function ppb_setClipScale(argsJson) {
  try {
    var args;
    try {
    ...
    if (typeof args.scale !== "number") {
      return JSON.stringify({ ok: false, error: "scale must be a number (100 = original size)" });
    }
    ...
    var setResult = setComponentProperty(motion, ["Scale"], args.scale);
    if (!setResult.found) {
      return JSON.stringify({ ok: false, error: "Scale property not found on the Motion component" });
    }
