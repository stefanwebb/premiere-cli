// Command: set-clip-anchor-point → ppb_setClipAnchorPoint
...
// premiere-pro-mcp `set_clip_anchor_point` tool (track-targeting.ts),
...
// Anchor Point property ([x, y] in pixels). Mutating — undo is
// non-functional on this build, so previousValue in the result is the
// caller's only restoration path.

function ppb_setClipAnchorPoint(argsJson) {
  try {
    var args;
    try {
    ...
    var setResult = setComponentProperty(motion, ["Anchor Point"], [args.x, args.y]);
    if (!setResult.found) {
      return JSON.stringify({ ok: false, error: "Anchor Point property not found on the Motion component" });
    }
