// Command: deselect-all-clips → ppb_deselectAllClips
// Loaded lazily by ppb_dispatch (host/index.jsx) on first use, into the
// same global ExtendScript context — shared helpers (findSequenceByName,
// timeValueToSeconds, ...) are already defined there.
//
// Standard-DOM only (clip.setSelected(false, true)) — no QE needed. Ported
// from leancoderkavy's premiere-pro-mcp `deselect_all_clips` tool
// (selection.ts). Mutation — verified via seq.getSelection() afterward
// (expected to read back empty).

function ppb_deselectAllClipsCollectSelection(seq) {
  var sel = null;
  try {
...
function ppb_deselectAllClips(argsJson) {
  try {
    var args;
    try {
    ...
    var selectionResult = ppb_deselectAllClipsCollectSelection(seq);
